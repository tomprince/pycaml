...
(* DDD *)
val _mldump : 'a -> unit
val _pyrun_string_token0 : bool -> int
...
(** As Python is a dynamically typed language, [pyobject] values
    ...
    of types that have been made known to OCaml. To leave the type
    deliberately unspecified, use "AnyType" (this is used in the
    function pyrefcount, for example). The default for "unknown"
    values is OtherType.
...
val py_mod_main: pyobject
val py_mod_main_dict: pyobject
...
val python_object_from_module : string -> string -> pyobject
...
val get_submodule_dict :
  ?genesis_modulename:string -> pyobject -> string -> pyobject

val py_register :
  ?create_modules:bool -> string -> pyobject -> unit

val py_resolve : string -> pyobject

(** XXX FIX DOCUMENTATION XXX

    This helper simplifies the creation of OCaml callbacks that can be
    registered in a Python module.

    First argument: An array of [pyobject_type] Python types. These
    are the expected types of the arguments to the function.

    Second argument: A "body" function B mapping an OCaml array
    of Python values to a Python return value.

    The body function (as well as the optional checks, see below) will
    be wrapped up in code that first checks for the correct number and
    the specified Python types of arguments, so B can rely on the n'th
    entry of its Python argument array being of the Python type
    specified in the n'th position of the type array.


    Optional arguments:

    register_as -- String of the form '[[[module.]submodule.]...]fun_name'.
       If provided, adds the function under the given name to the
       specified module.

    error_name -- String which will be used in both profiling and
       error reporting. Defaults to register_as (if provided).

    catch_weird_exceptions -- If true then any exception raised on the
       OCaml level which cannot be handled is caught gracefully, with
       an error message being displayed. If false then the exception is
       raised anyway. Default value: true.

    docstring -- Docstring which is added to the python function.

    extra_guards -- An array of extra checks to be performed on the
       arguments, one by one, returning an optional error message.
...
val python_interfaced_function :
  ...
  pyobject_type array -> (pyobject array -> pyobject) -> pyobject
...
val guarded_pystring_asstring: pyobject -> string
...
*)
val python_exec : string -> int

val python_eval : ?print:bool -> ?like_repl:bool -> ?globals:pyobject -> ?locals:pyobject -> string -> pyobject
...
val pyerr_fetch_broken :
  pyobject * pyobject * pyobject -> pyobject * pyobject * pyobject

val pyerr_fetch : unit -> (pyobject * pyobject * pyobject)
...
val pylist_ftensor_new:
 ?init:(int array -> float) -> int array ->
  pyobject * (int array -> pyobject -> unit);;
val pylist_tensor_shape: pyobject -> int list;;
...
module Nicerpy :
  sig
    ...
    val profiler_on : unit -> bool
    val profiler_off : unit -> bool
    val profiler_reset : unit -> unit
    val profiler_report : unit -> (string * float * float) array
    val py_to_type : pyobject -> pyobject_type
    val repr : pyobject -> string
    val pylist_to_array : pyobject -> pyobject array
    val pylist_set : pyobject -> int -> pyobject -> unit
    val pylist_get : pyobject -> int -> pyobject
    val py_result : unit -> pyobject
    val py_true : pyobject
    val py_false : pyobject
    val py_null : pyobject
    val py_none : pyobject
    val py_to_bool : pyobject -> bool
    val py_register : ?create_modules:bool -> string -> pyobject -> unit
    val py_resolve : string -> pyobject
    val define_pill_types : string array -> unit
    val pill_to_type : pyobject -> 'a
    val pill_wrapper_unwrapper :
      string -> 'a -> ('a -> pyobject) * (pyobject -> 'b)
    val py_optionally : (pyobject -> 'a) -> pyobject -> 'a option
    val py_to_int : pyobject -> int
    val py_to_float : pyobject -> float
    val py_to_string : pyobject -> string
    val bool_to_py : bool -> pyobject
    val int_to_py : int -> pyobject
    val float_to_py : float -> pyobject
    val string_to_py : string -> pyobject
    val fun_to_py :
      ?register_as:string ->
      ?error_name:string ->
      ?catch_weird_exceptions:bool ->
      ...
      ?extra_guards:(pyobject -> string option) array ->
      pyobject_type array -> (pyobject array -> pyobject) -> pyobject
    val py_to_fun : pyobject -> pyobject array -> pyobject
    val id : 'a -> 'a
    val py_to_array : (pyobject -> 'a) -> pyobject -> 'a array
    val array_to_pylist : ('a -> pyobject) -> 'a array -> pyobject
    val array_to_pytuple : ('a -> pyobject) -> 'a array -> pyobject
    val ht_new : unit -> pyobject
    val ht_get : ?default:pyobject -> pyobject -> pyobject -> pyobject
    val ht_set : pyobject -> pyobject -> pyobject -> int
    val ht_del : pyobject -> pyobject -> int
    val ht_clear : pyobject -> unit
    val ht_keys : pyobject -> pyobject array
    val ht_len : pyobject -> int
    val py_exec : string -> int
    val py_eval :
      ?print:bool ->
      ?like_repl:bool ->
      ?globals:pyobject -> ?locals:pyobject -> string -> pyobject
    val py_load : string -> unit
    val py_repl : unit -> int
    val py_main : string array -> int
    val ipython : unit -> int
  end

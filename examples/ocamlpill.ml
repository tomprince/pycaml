...
(within OCaml) into what is called an "OCamlPill". This essentially
means that to Python it is a "pill" with unknown content whereas OCaml
can unwrap it and use the content if the pill is given back to it.
...
'pysym_type' so that we can change the actual string later. *)
...
(* Now we write the function that creates the data structure and gives it to Python as an OCamlPill:*)
let _py_create_pill =
  python_interfaced_function 
    ~docstring:"Create my pill in OCaml and retrieve it from OCaml (to Python).\nFunction takes three integers."
...
let _py_do_stuff_with_pill =
  python_interfaced_function
    ~docstring:"Takes an ocaml pill of type 'example' and returns the first integer in that pill"
    [|CamlpillType|]
    (fun py_args -> 
...
in register_for_python [|"do_stuff",_py_do_stuff_with_pill|];; 
...
Finally, to feed the 'pill' back to OCaml, do something with it in OCaml, and 
...
let _py_do_stuff_with_pill2 = python_interfaced_function
    ~docstring:"Takes an ocaml pill and an int, and returns the sum of all ints in the pill and the new int"
...
in register_for_python [|"compute_sum_offset",_py_do_stuff_with_pill2|];;

let () = Printf.printf "From within Python, try the following:\n\n"
and () = Printf.printf " >>> a=ocaml.create_pill(1,3,4)\n"
and () = Printf.printf " >>> b=ocaml.compute_sum_offset(a,10)\n\n"
in
Printf.printf "b should contain the sum of 1, 3, 4, 99 and 10.\n\n%!";;

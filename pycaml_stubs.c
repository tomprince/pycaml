...
value pygencall( value format, value arg ) {
  CAMLparam2(format,arg);
    ...
    case 4:
      CAMLreturn(Val_int(((type_4)func)()));

    case 5:
      CAMLreturn(Val_int(((type_5)func)
			 (String_val(arg))));
	...
	*/
    ...
    case 24:
      CAMLreturn(Val_int(((type_24)func)
			 (pyunwrap(Field(arg,0)),
			  pyunwrap(Field(arg,1)),
			  pyunwrap(Field(arg,2)))));
...
value _mldump(value obj) {
  CAMLparam1(obj);
  fprintf(stderr,"CAML DUMP: %p\n",(void *) obj);
  fflush(stderr);
  CAMLreturn(Val_unit);
}
...
value pytype( value obj ) {
    ...
    if( !pobj ) CAMLreturn(Val_int(NullType));
...
/* Auxiliary function: return either Py_single_input or Py_eval_input
   
   This must be passed back to pyrun_string to tell the interpreter
   how to handle the string to be executed.
 */
value _pyrun_string_token0(value ml_like_repl) {
  CAMLparam1(ml_like_repl);
  CAMLreturn(Val_int(Bool_val(ml_like_repl)?Py_single_input:Py_eval_input));
}

value pyerr_fetch_wrapped(value ml_unit) {
  CAMLparam1(ml_unit);
  CAMLlocal1(rv);
  ...
  rv = alloc_tuple(3);
  Store_field(rv,0,pywrap_steal(ptype));
  Store_field(rv,1,pywrap_steal(pval));
  Store_field(rv,2,pywrap_steal(ptraceback));

  CAMLreturn(rv);
}

value pycaml_pymain(value ml_argv) {
  CAMLparam1(ml_argv);
  int argc;
  char ** argv;
  int updatepath;
  ...
  argc = Wosize_val(ml_argv);

  /* The following code is partly stolen from mpi_petsc/petsc_stubs.c
     (line 389 ff). Perhaps it makes sense to unify this somehow if
     possible. */

  if(NULL==(argv=malloc((1+argc)*sizeof(char*))))
    {
      fprintf(stderr,"malloc() failure in function pycaml_pymain!\n");
      exit(1);
    }

  for(i=0;i<argc;i++)
    {
      if(NULL==(argv[i]=strdup(String_val(Field(ml_argv,i)))))
	{
	  fprintf(stderr,"malloc() failure in function pycaml_pymain!\n");
	  exit(1);
	}
